...
def get_planes_params(
    ...
    z_positions_full_planes = da.mean(
        stage_positions_da[:, channels_full_planes, :, 2], axis=0
    ).compute()
...
def roll_single_planes(
    ...
    z_positions_one_plane = da.mean(
        stage_positions_da[:, channels_one_plane, 0, 2], axis=0
    ).compute()
...
def _fuse_xy(
    ...
    if "" not in fns:
        ...
        if img_shape is not None and img_dtype is not None:
            img = np.zeros(img_shape, dtype=img_dtype)
        else:
            raise RuntimeError(
                "'img_dimension' and 'img_dtype' must be provided if plane is" " empty"
            )
...
def get_well_image_CZYX_lazy(
    ...
    z_sampling, min_z, n_z = get_planes_params(stage_positions_da, channels_full_planes)
    ...
    imgs_fused_da = da.map_blocks(
        _fuse_xy,
        da.from_array(
            fns_np, chunks=((fns_np.shape[0],) + (1,) * len(fns_np.shape[1:]))
        ),
        chunks=da.core.normalize_chunks(
    ...
    for c in range(fns_np.shape[1]):
        if (c in channels_full_planes) or (c in channels_one_plane):
            for z in range(fns_np.shape[2]):
                if fns_np[0, c, z] != "":
                    channel_metadata.append(load_channel_metadata(fns_np[0, c, z]))
                    break
        else:
    ...
    channel_histograms = load_UIntHistograms(
        imgs_fused_da[:, [imgs_fused_da.shape[1] // 2]], channels_empty
    )
